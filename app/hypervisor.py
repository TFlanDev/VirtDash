...
def get_vm_info(conn, name: str):
    """
    Finds a single VM by name and returns its name, state, and memory.
    """
    try:
        # Use lookupByName to find a specific domain
        ...
        print(f"Error: Could not find VM with name '{name}'.", file=sys.stderr)
        return None
